// FIXED MARKET MODULE - Processes ALL items, not just the first one
// This works with "Run Once for All Items" mode

try {
  console.log("=== FIXED MARKET MODULE START ===");
  
  // Get all input items
  const allItems = $input.all();
  console.log("Processing", allItems.length, "leads for market scoring");
  ...
  const processedItems = allItems.map((item, index) => {
    ...
    const competitorCount = utils.validateNumericInput(item.json.competitor_count, 0, 0, 100);
    ...
    const populationDensity = utils.validateNumericInput(item.json.population_density, 0, 0, 50000);
    ...
    let growthScore = 0;
    const marketGrowthRate = utils.validateNumericInput(item.json.market_growth_rate, 0, -50, 100);
    ...
    const unemploymentRate = utils.validateNumericInput(item.json.unemployment_rate, 5, 0, 50);
    
    if (unemploymentRate <= 3) {
      unemploymentScore = 50;
    ...
    const incomeTrend = item.json.income_trend || 'stable';
    ...
    let maturityScore = 0;
    const marketMaturity = item.json.market_maturity || 'mature';
  ...
  console.log("=== FIXED MARKET MODULE END ===");
  ...
  console.error('Error in Fixed Market Module:', error);
  ...
  return allItems.map((item) => ({
    json: {
      ...item.json,
      market_score: 0,
      market_reasons: [`Error in market calculation: ${error.message}`],
      error: error.message
    }
  }));
}
